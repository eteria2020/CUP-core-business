 ...
 */
class Version20160607115355 extends AbstractMigration
{
    const MAIN_TABLE = 'business.time_package_payment';
    const SEQUENCE_NAME = 'business.time_package_payment_id_seq';

    const SUPPORT_TABLE = 'business.time_package_payment_transaction';
    ...
    public function up(Schema $schema)
    {
        ...
        $table->addColumn("time_package_payment_id", "integer", ["notull" => true]);
        $table->addColumn("transaction_id", "integer", ["notull" => true]);
        $table->addColumn("created_ts", "datetime", ["notnull" => true, "default" => 'CURRENT_TIMESTAMP']);
        $table->setPrimaryKey(["time_package_payment_id", "transaction_id"]);
        $table->addForeignKeyConstraint('business.time_package_payment', ['time_package_payment_id'], ['id']);
