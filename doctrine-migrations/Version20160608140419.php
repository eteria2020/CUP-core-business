 ...
 */
class Version20160608140419 extends AbstractMigration
{
    const MAIN_TABLE = 'business.business_trip_payment';
    const SEQUENCE_NAME = 'business.business_trip_payment_id_seq';

    const SUPPORT_TABLE = 'business.business_trip_payment_transaction';
    ...
    public function up(Schema $schema)
    {
        ...
        $table->addColumn("business_trip_id", "integer", ["notnull" => true]);

        $table->setPrimaryKey(["id"]);
        $table->addForeignKeyConstraint('business.business', ['business_code'], ['code']);
        $table->addForeignKeyConstraint('business.business_trip', ['business_trip_id'], ['id']);
        $table->addForeignKeyConstraint('business.business_invoice', ['invoice_id'], ['id']);

        $table = $schema->createTable(self::SUPPORT_TABLE);
        $table->addColumn("business_trip_payment_id", "integer", ["notull" => true]);
        $table->addColumn("transaction_id", "integer", ["notull" => true]);
        $table->addColumn("created_ts", "datetime", ["notnull" => true, "default" => 'CURRENT_TIMESTAMP']);
        $table->setPrimaryKey(["business_trip_payment_id", "transaction_id"]);
        $table->addForeignKeyConstraint('business.business_trip_payment', ['business_trip_payment_id'], ['id']);
