 ...
 */
class Version20160613154841 extends AbstractMigration
{
    const MAIN_TABLE = 'business.subscription_payment';
    const SEQUENCE_NAME = 'business.subscription_payment_id_seq';

    const SUPPORT_TABLE = 'business.subscription_payment_transaction';
    ...
    public function up(Schema $schema)
    {
        ...
        $table->addColumn("subscription_payment_id", "integer", ["notull" => true]);
        $table->addColumn("transaction_id", "integer", ["notull" => true]);
        $table->addColumn("created_ts", "datetime", ["notnull" => true, "default" => 'CURRENT_TIMESTAMP']);
        $table->setPrimaryKey(["subscription_payment_id", "transaction_id"]);
        $table->addForeignKeyConstraint('business.subscription_payment', ['subscription_payment_id'], ['id']);
